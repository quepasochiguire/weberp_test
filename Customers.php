		...
		if (!isset($_POST['New'])) {
			...
			if ($myrow[0] == 0) {
			  $sql = "UPDATE debtorsmaster SET	name='" . $_POST['CustName'] . "',
												...
												typemayorist_id='" . $_POST['WholeCode'] . "'
					  WHERE debtorno = '" . $_POST['DebtorNo'] . "'";
			} else {
...
// Show Customer Type drop down list
	...
	$result=DB_query("SELECT id, code, name FROM typemayorist");
	if (DB_num_rows($result)==0){
		$DataError =1;
		echo '<tr>
				<td colspan="2">' . prnMsg(_('There are no wholesaler type defined - go to the setup tab of the main menu and set at least one up first'),'error') . '</td>
			</tr>';
	} else {
		if (!isset($_POST['WholeCode'])){
			$CurrResult = DB_query("SELECT id, code, name FROM typemayorist");
			$myrow = DB_fetch_row($CurrResult);
			$_POST['WholeCode'] = $myrow[0];
		}
		echo '<tr>
				<td>' . _('Type of wholesaler') . ':</td>
				<td><select tabindex="17" name="WholeCode" required="required">';
		while ($myrow = DB_fetch_array($result)) {
			if ($_POST['WholeCode']==$myrow['id']){
...
//DebtorNo exists - either passed when calling the form or from the form itself
	...
	if (!isset($_POST['New'])) {
		...
		$_POST['WholeCode'] = $myrow['typemayorist_id'];
...
// Select Customer types for drop down list for SELECT/UPDATE
	...
	if (isset($_GET['Modify'])) {
		$result=DB_query("SELECT id, code, name FROM typemayorist WHERE id='".$_POST['WholeCode']."'");
		$myrow=DB_fetch_array($result);
		
		echo '<tr>
				<td>' . _('Type of wholesalerr') . ':</td>
				<td>' . $myrow['name'] . '</td>
			</tr>';
	} else {
		$result=DB_query("SELECT id, name FROM typemayorist");
		echo '<tr>
				<td>' . _('Type of wholesalerr') . ':</td>
				<td><select name="WholeCode" required="required">';
		while ($myrow = DB_fetch_array($result)) {
			if ($_POST['WholeCode'] == $myrow['id']){
				echo '<option selected="selected" value="'. $myrow['id'] . '">' . $myrow['name'] . '</option>';
			} else {
