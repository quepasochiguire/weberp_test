...
if(isset($_GET['Area'])) {
	...
	$_POST['CustAdd'] = '';
	$_POST['CustType'] = '';
	$_POST['WholeCode'] = $_GET['WholeCode'];
}
...
if(isset($_POST['Search']) OR isset($_POST['CSV']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
	...
	if(($_POST['Keywords'] == '') AND ($_POST['CustCode'] == '') AND ($_POST['CustPhone'] == '') AND ($_POST['CustType'] == 'ALL') AND ($_POST['Area'] == 'ALL') AND ($_POST['CustAdd'] == '') AND ($_POST['WholeCode'] == '')) {
		...
		if(mb_strlen($_POST['WholeCode'] != '0')) {
			$SQL .= " AND debtorsmaster.typemayorist_id = '" . (int)$_POST['WholeCode'] . "'";
		}
...
/* Option to select a type wholesaler */
echo '<td><b>', _('OR'), '</b></td>
		<td>' . _('Type of wholesaler') . ':</td><td>';
$result2 = DB_query("SELECT id, code, name FROM typemayorist");
// Error if no sales areas setup
if(DB_num_rows($result2) == 0) {
	$DataError = 1;
	echo '<a href="TipoMorista.php" target="_parent">' . _('Setup Type of wholesaler') . '</a>';
	echo '<tr><td colspan="2">' . prnMsg(_('No Type of wholesaler defined'), 'error') . '</td></tr>';
} else {
	// if OK show select box with available options to choose
	echo '<select name="WholeCode">';
	echo '<option value="0">' . _('Any') . '</option>';
	while ($myrow = DB_fetch_array($result2)) {
		if(isset($_POST['WholeCode']) AND $_POST['WholeCode'] == $myrow['id']) {
		...
		echo '<table cellpadding="2" class="selection">
				<thead>
					<tr>
						...
						<th class="ascending">' . _('Wholesaler') . '</th>
					</tr>
				</thead>';
		$RowIndex = 0;
	...
	if(DB_num_rows($result) <> 0) {
		...
		while (($myrow = DB_fetch_array($result)) AND ($RowIndex <> $_SESSION['DisplayRecordsMax'])) {
			echo '<tr class="striped_row">
				...
				<td class="text">', $myrow['typemayorist_id'], '</td>
			</tr>';
			$i++;
			$RowIndex++;
